...
var runScopedAWSPrefixPattern = regexp.MustCompile(`^[a-z]{2,3}-r[a-z0-9]{8}$`)
...
func NormalizeAWSPrefix(value string) (string, error) {
	...
	if !awsPrefixPattern.MatchString(prefix) && !runScopedAWSPrefixPattern.MatchString(prefix) && !automationAWSPrefixPattern.MatchString(prefix) {
...
func IsRunScopedAWSPrefix(value string) bool {
	return runScopedAWSPrefixPattern.MatchString(strings.ToLower(strings.TrimSpace(value)))
}
