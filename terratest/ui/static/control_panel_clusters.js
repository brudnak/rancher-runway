...
export const createClusterPanel = ({
  ...
  const initializeCollapseState = cluster => {
    ...
    if (cluster.type === 'local' && cluster.gpuWorkerIp && cluster.deploymentType !== 'hosted-tenant-k3s' && cluster.deploymentType !== 'linode-docker-cattle') {
  ...
  const renderGPUWorkerPanel = cluster => {
    if (cluster.type !== 'local' || cluster.deploymentType === 'hosted-tenant-k3s' || cluster.deploymentType === 'linode-docker-cattle' || !cluster.gpuWorkerIp) {
      return ''
    }
...
kubectl -n cattle-ai-agent-system get pods -o wide`
  ...
  const renderCluster = cluster => {
    ...
    const gpuWorker = !isDownstream && !isHostedTenant && !isLinodeDocker && cluster.gpuWorkerIp
      ? `
        <div class="space-y-1">
          <div>${escapeHtml(cluster.gpuWorkerIp)}</div>
          <div class="text-xs text-zinc-500 dark:text-zinc-400">${escapeHtml(cluster.gpuWorkerInstanceType || 'GPU worker')} RKE2 agent${cluster.gpuWorkerPrivateIp ? ` • private ${escapeHtml(cluster.gpuWorkerPrivateIp)}` : ''}</div>
        </div>
      `
      : ''
      ...
      <article class="min-w-0 overflow-hidden rounded-2xl border border-zinc-200 ${downstreamClasses} p-4 shadow-sm dark:border-white/10">
        ...
        ${clusterCollapsed ? '' : `
          <div class="mt-4 grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-[repeat(3,minmax(0,1fr))]">
            ...
            ${gpuWorker ? metaItem('GPU worker', gpuWorker) : ''}
            ${namespace}
            ${clusterID}
          </div>
          ${leaderSummary}
