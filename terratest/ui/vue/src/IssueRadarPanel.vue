...
<template>
  <div class="issue-radar">
    ...
    <form class="ir-config" @submit.prevent="runReport" :aria-busy="loading">
      ...
      <p v-if="error" class="ir-alert ir-alert-error" role="alert">{{ error }}</p>
    </form>
    ...
    <template v-else>
      ...
      <p v-if="message && view !== 'report'" class="ir-alert" :class="{ 'ir-alert-error': messageError }" role="status">{{ message }}</p>
    </template>
  </div>
</template>
