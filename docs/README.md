...
- `plan-rancher-regression.yml`: manual regression-only launcher that starts
  the full planner with a locked `framework-regression` filter and waits for
  the planner to finish.
